...
class SAINT(nn.Module):
    def __init__(self, n_quest, n_part, n_responses, embed_dim=64, max_seq=MAX_SEQ, device = 'cpu'):
        ...
        self.output = nn.Linear(in_features=embed_dim, out_features=2)
        self.max_seq=max_seq
...
def train_epoch():
    ...
    mean_loss=torch.mean(torch.stack(train_loss))
    return mean_loss
...
for i in range(num_epochs):
    ...
    print("Epoch:{} | Train Loss: {}".format(i, train_loss.item()))
